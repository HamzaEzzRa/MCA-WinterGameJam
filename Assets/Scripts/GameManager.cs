...
public class GameManager : MonoBehaviour
{
    ...
    [SerializeField] private GameObject scorePrefab = default;
    [SerializeField] private GameObject scoreBorderPrefab = default;
    [SerializeField] private GameObject leaderBoardContent = default;

    [SerializeField] private GameObject facebookButton = default;
    [SerializeField] private GameObject debugPanel = default;
    ...
    private void Awake()
    {
        ...
        if (Utility.Platform != RuntimePlatform.Android && Utility.Platform != RuntimePlatform.IPhonePlayer && facebookButton != null)
    ...
    private void PauseGame()
    {
        ...
        for (int i = 0; i < UICanvas.transform.childCount; i++)
        {
            ...
            if (childTransform.gameObject != healthBar && childTransform.gameObject != optionsPanel &&
                ...
                childTransform.gameObject != facebookButton && childTransform.gameObject != debugPanel)
                childTransform.gameObject.SetActive(true);
            else
                childTransform.gameObject.SetActive(false);
        }
    ...
    public void GameOver()
    {
        ...
        if (CotcManager.Instance.Gamer != null)
        {
            long personalScore = InGameUI.Instance.GetStopwatchScore();
            CotcManager.Instance.PostScore(personalScore, CotcSdk.ScoreOrder.LowToHigh).Then(postedScore =>
            {
                CotcManager.Instance.GetBestHighScores().Then(bestScoreResult =>
                {
                    foreach (CotcSdk.Score score in bestScoreResult)
                    {
                        GameObject scoreObject = Instantiate(scorePrefab, leaderBoardContent.transform);
                        if (score.GamerInfo.GamerId == CotcManager.Instance.Gamer.GamerId)
                            Instantiate(scoreBorderPrefab, scoreObject.transform);

                        TMPro.TextMeshProUGUI[] texts = scoreObject.GetComponentsInChildren<TMPro.TextMeshProUGUI>();

                        texts[0].SetText(score.Rank.ToString());

                        TimeSpan timeSpan = TimeSpan.FromMilliseconds(score.Value);
                        texts[1].SetText(timeSpan.ToString(@"mm\:ss\:fff"));
                    }

                    CotcManager.Instance.GetScoreRank(personalScore).Then(rank =>
                    {
                        if (rank > 10)
                        {
                            CotcManager.Instance.GetCenteredScores().Then(centeredScoreResult =>
                            {
                                foreach (CotcSdk.Score score in centeredScoreResult)
                                {
                                    GameObject scoreObject = Instantiate(scorePrefab, leaderBoardContent.transform);
                                    if (score.GamerInfo.GamerId == CotcManager.Instance.Gamer.GamerId)
                                        Instantiate(scoreBorderPrefab, scoreObject.transform);
